...
source("./utils.r")
source("./methods/spectral_regularized.R")
source("./metrics.r")

train_rsc <- function(N, K, beta, b, seed){
  # generate data
  ...
  # spectral clustering
  tic()
  res <- regularized_spectral_clustering(A, K)
  time <- toc()
...
rsc <- foreach(i=1:m,.combine=rbind,
                                ...
                                for (b in b_list) {
                                  for (K in K_list) {
                                    for (beta in beta_list) {
                                      res <- rbind(res, train_rsc(N, K, beta, b, i))
                                    }
                                  }
                                }
                              }
