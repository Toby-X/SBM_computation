...
source("./utils.r")
source("./methods/spectral_null_simplex.R")
source("./metrics.r")

train_ss <- function(N, K, beta, b, seed){
  # generate data
...
ss <- foreach(i=1:m,.combine=rbind,
                                ...
                                for (b in b_list) {
                                  for (K in K_list) {
                                    for (beta in beta_list) {
                                      res <- rbind(res, train_ss(N, K, beta, b, i))
                                    }
                                  }
                                }
                              }
