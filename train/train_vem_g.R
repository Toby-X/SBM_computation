...
train_vem <- function(N, K, beta, b, seed){
  ...
  # spectral clustering
  tic()
  res <- blockmodels:::BM_gaussian("SBM", A, explore_min=K-1, explore_max=K)
  try(res$estimate())
  time <- toc()
  time <- time$toc - time$tic
  Z_hat <- t(apply(res$memberships[[K]]$Z, 1, function(x) as.integer(x == max(x))))
  B_hat <- res$model_parameters[[K]]$mu
