...
source("./utils.r")
source("./methods/spectral_score.R")
source("./metrics.r")

train_sscore <- function(N, K, beta, b, seed){
  # generate data
  ...
  # spectral clustering
  tic()
  res <- spectral_simplex_score(A, K)
  time <- toc()
                                ...
                                for (b in b_list) {
                                  for (K in K_list) {
                                    for (beta in beta_list) {
                                      res <- rbind(res, train_sscore(N, K, beta, b, i))
                                    }
                                  }
                                }
                              }
